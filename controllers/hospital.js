...
exports.signupHospital = asyncHandler(async (req, res, next) => {
  ...
  res.status(201).json({
    success: true,
    message:
      'Hospital registered successfully. Please verify your email using the OTP sent.',
  })
})
// ------------------- signup --------------------
...
exports.exportExcel = asyncHandler(async (req, res, next) => {
  ...
  if (hospital?.length === 0)
    return next(new ErrorHandler('Select Order First'))
...
exports.importExcel = asyncHandler(async (req, res, next) => {
  if (!req?.file?.path)
    return next(new ErrorHandler('Please select file first'))
  const workbook = new ExcelJS.Workbook()
  ...
  res
    .status(200)
    .json({ message: 'Data imported successfully', count: hospitals.length })
})
//import